...
public class StartingCharacteristicsFragment extends Fragment {
    ...
    private void setNewValue(int value, int ID) {
        ...
        binding.characteristicsPoints.setText(String.format("Осталось очков характеристик: %d", getActivity().getPreferences(Context.MODE_PRIVATE).getInt(Constants.Characteristics_Points, 2)));
    }
    ...
    private void settingSkillsFragment() {
        if (getActivity().getPreferences(Context.MODE_PRIVATE).getInt(Constants.Characteristics_Points, 2) > 0) {
